...
- `scripts/maintenance_uploader.py`：按轮次和时间窗口执行备份、上传节奏控制。
- `scripts/maintenance_uploader_scheduler.ps1`：本地轮询启动器（默认每 60 秒）。
- `scripts/maintenance_uploader_schedule_task.ps1`：Windows 任务计划器装配脚本（安装/查询/卸载）。
...
- 按时间窗口推进：小版本每 2 轮形成候选，大版本每 10 轮形成候选；优先在同一窗口完成本地连续优化，窗口结束后统一提交。
- 公开仓库节奏保持可见：不追求一次推满，保留 2 小时粒度的进展提交。
- 版本节奏：每 2 轮形成小版本候选，每 10 轮形成大版本候选；以 `v0.1.x` 连续递增记录进展。
